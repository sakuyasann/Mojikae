...
- **言語・種類で絞り込む**（日本語 × セリフ、など）
- **フォントの組み合わせをプリセットとして保存する**
...
- **保存するのはフォント名まわりだけです。** 具体的には次の 2 つで、いずれもサイトとは無関係です。
  - 最近使用したフォント名（最大 5 件）
  - プリセット（プリセット名とフォント名の並び。最大 20 件）

  サイトの URL・タイトル・タブ情報・適用対象・適用状態は一切保存しません。
  閲覧情報の収集・送信は行いません。
...
検索欄の下に出る**候補パネルの中**に「言語」「種類」の 2 つの絞り込みを置いています。
常時表示にするとポップアップの縦幅を 50px 以上圧迫するため、候補を開いたときだけ現れます。

- **言語**: `subsets` を見て判定します。カタログには 181 種類のサブセットがありますが、
  実際に選ぶ対象になるものだけを、日本語のフォント選定で使う頻度の高い順に並べています
  （日本語 / ラテン / 韓国語 / 中国語 / キリル など 13 種類）
- **種類**: `category`（サンセリフ / セリフ / 等幅 / ディスプレイ / 手書き）

選択肢の件数は**もう一方の絞り込みを適用した状態**で数えるので、「日本語」を選ぶと
種類の件数が日本語フォントだけの内訳（サンセリフ 29 / セリフ 20 / 等幅 1 …）に変わり、
0 件になる選択肢は表示されません。

UI はネイティブの `select` を macOS の pop-up button 風に見せているだけなので、
キーボード操作と支援技術への対応は標準のまま効きます。
...
選択中のフォントの組み合わせに名前を付けて保存し、あとから呼び出せます。

- 保存するのは**プリセット名とフォント名の並び**だけです。並び順も保つので、
  「英字は Playfair Display、日本語は Zen Maru Gothic」といった組み合わせをそのまま再現できます
- 名前を省略すると組み合わせから自動生成します（例: `Playfair Display + Zen Maru Gothic`）
- 同じ組み合わせを保存し直すと、重複を作らず名前を更新して先頭へ移します
- 最大 20 件

**サイトには紐付けていません。** 当初の仕様で「サイトごとの設定保存」を対象外とし、
プライバシー方針でもサイト情報を保存しないと定めているためです。
サイトごとに自動適用したい場合は、URL を保存する設計になるので別途判断が必要です。
...
ブラウザのポップアップは最大 600px までしか高くできない（Chrome / Firefox 共通）ため、
その中へ収まるように「適用対象」を既定で折りたたんでいます。

- 既定は「ページ全体」の行だけを表示（この状態ならポップアップは 470px 前後）
- 「ページ全体」のチェックを外すと、検出したフォントを**全件展開**する
- 選択中のフォントも全件表示する

リストの内側ではスクロールさせず、収まらない場合は外側がスクロールします。
