...
import { EMPTY_TAB_STATE, FONT_NAME_SEPARATOR, type TabApplyState } from '../types/tab-state';
...
export function readPageState(args: ReadStateArgs): TabApplyState {
  ...
  const fontsAttr = root.getAttribute(args.attrs.font) ?? '';

  return {
    active,
    fontFamilies: fontsAttr
...
export async function readTabState(tabId: number): Promise<TabApplyState> {
  const [injection] = await browser.scripting.executeScript({
    target: { tabId },
    func: readPageState,
    args: [{ attrs: PAGE_ATTRS, activeValue: ACTIVE_VALUE, separator: FONT_NAME_SEPARATOR }],
  });
